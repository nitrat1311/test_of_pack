  ...
  Future<void> onLoad() async {
    ...
    if (!_isAlreadyLoaded) {
      ...
      spriteSheet = SpriteSheet.fromColumnsAndRows(
        image: images.fromCache('football_garbage.png'),
        columns: 8,
        rows: 1,
      );
      ...
      animationRight = SpriteSheet.fromColumnsAndRows(
        ...
        columns: 3,
        rows: 1,
      ).createAnimation(from: 0, to: 3, row: 0, stepTime: 0.15, loop: false);
      fire = SpriteSheet.fromColumnsAndRows(
        image: images.fromCache('animation_fire.png'),
        columns: 6,
        rows: 1,
      ).createAnimation(from: 0, to: 5, row: 0, stepTime: 0.1, loop: true);
      ...
      add(player);
      _healthBar = HealthBar(player: player, position: Vector2(25, 55));
      add(_healthBar);
      ...
      _playerScore = TextComponent(
        position: Vector2(size.x - 208, 30),
      ...
      _playerScore2 = TextComponent(
        position: Vector2(size.x - 210, 28),
        textRenderer: TextPaint(
            style: TextStyle(
                letterSpacing: 5,
                fontFamily: 'JacquesFrancois',
