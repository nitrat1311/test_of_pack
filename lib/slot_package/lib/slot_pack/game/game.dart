...
class Level1Page extends Component with HasGameRef<MasksweirdGame> {
  ...
  late Fense fence;
  late Sprite sprite;
  late SpriteSheet allySpriteSheet;
  ...
  Future<void> onLoad() async {
    ...
    if (!_isAlreadyLoaded) {
      ...
      allySpriteSheet = SpriteSheet.fromColumnsAndRows(
        image: gameRef.images.fromCache('allySprites.png'),
        columns: 4,
        rows: 2,
      );
      ...
      _allyManager = AllyManager(spriteSheet: allySpriteSheet);
      add(_allyManager);
      add(_enemyManager);
...
class Level3Page extends Component with HasGameRef<MasksweirdGame> {
  late Player player;
  late Fense fence;
  late Sprite sprite;
  late SpriteSheet allySpriteSheet;
  ...
  Future<void> onLoad() async {
    ...
    if (!_isAlreadyLoaded) {
      ...
      await game.images.loadAll([
        'animation_fire3.png',
        'animation_right3.png',
        'animation_forward3.png',
        'ally3.png',
        'allySprites.png'
      ]);
      ...
      _background = Background(gameBackPath: 'game_back3.png');
      await add(_background);
      sprite = Sprite(gameRef.images.fromCache('ally3.png'));
      allySpriteSheet = SpriteSheet.fromColumnsAndRows(
        image: gameRef.images.fromCache('allySprites.png'),
        columns: 4,
        rows: 2,
      ...
      no_fire = SpriteSheet.fromColumnsAndRows(
        image: gameRef.images.fromCache('animation_right3.png'),
        columns: 4,
        rows: 1,
      ).createAnimation(from: 0, to: 1, row: 0, stepTime: 0.2, loop: false);

      fire = SpriteSheet.fromColumnsAndRows(
        image: gameRef.images.fromCache('animation_fire3.png'),
        columns: 6,
        rows: 1,
      ).createAnimation(from: 0, to: 5, row: 0, stepTime: 0.25, loop: true);
      // Create a basic joystick component on left.
      ...
      add(_healthBar2);
      player = Player(
        fireImgPath: 'animation_fire3.png',
        animationRightPath: 'animation_right3.png',
        animationForwardPath: 'animation_forward3.png',
        joystick: joystick,
        animation: no_fire,
        size: Vector2(179 / 2, 250 / 2),
        position: Vector2(0, gameRef.size.y / 1.9),
  ...
  StartPage() {
    addAll([
      ...
      _button3 = RoundedButton(
        text: 'Level 3',
        action: () => gameRef.router.pushNamed('level3'),
        color: const Color(0xFFF5543B),
        borderColor: const Color(0xfffff4c7),
      ),
    ]);
  }
